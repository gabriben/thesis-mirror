...
For the purpose of this thesis, we will call the combination of algorithms listed above the \emph{personalization pipeline} \mdr{What does this mean or add: (the platform's perspective)}. 
...
In this thesis, we propose individual tools that \mdr{what does this mean: map} to the user journey above, for the steps of \emph{recommendations}, \emph{thumbnail} selection, \emph{intent-satisfaction} linking, and \emph{diversity} measurement. 
Together, these tools form our proposed personalization pipeline. 
\mdr{Glue sentence needed that makes the next enumeration a natural one to make at this point in the paper. I suggest you include a diagram with these steps from the personalization pipeline and then explain what specific choices you're considering for these steps in the thesis.}
